...
// 保存状态行：平时隐藏，PUT 失败或加载失败时才露出来（成功即清空）
var saveStatusEl = document.createElement('div')
saveStatusEl.className = 'dshwv-menu-row'
saveStatusEl.style.display = 'none'
saveStatusEl.style.color = '#e0433f'
saveStatusEl.style.whiteSpace = 'normal'
menuBox.appendChild(saveStatusEl)
function setSaveStatus(text) {
  try {
    if (!text) { saveStatusEl.style.display = 'none'; saveStatusEl.textContent = ''; return }
...
var scrollGapPx = 17
// 加载门：服务端配置到达之前禁止 PUT，否则内存默认值会整包覆盖已存配置
// （重启后拖一下鲸鱼就洗掉全部设置的就是这个）。位置锚点不受影响，仍实时记 localStorage。
...
function saveConfig() {
  ...
  // 配置本体只在服务端数据到达后才写：加载完成前的任何改动只进内存+界面，不碰文件
  ...
  try {
    fetch(SIZE_URL, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ scale: state.scale, sound: soundOn, vol: soundVol, soundSet: soundSet, usageMode: usageMode, peakMode: peakMode, bubbleOn: bubbleOn, turnCostOn: turnCostOn, turnCostCloseMs: turnCostCloseMs, scrollGapOn: scrollGapOn, scrollGapPx: scrollGapPx }) })
      .then(function (r) { return r.json() })
      .then(function (d) {
        if (d && d.ok) { setSaveStatus('') }
        else {
          setSaveStatus('设置保存失败：服务端拒绝，本次修改仅生效到刷新')
          try { console.error('[dshw] save rejected', d) } catch (e) {}
        }
      })
      .catch(function (err) {
        setSaveStatus('设置保存失败：网络错误，本次修改仅生效到刷新')
        try { console.error('[dshw] save failed', err) } catch (e) {}
      })
  } catch (err) {}
}
...
fetch(SIZE_URL, { cache: 'no-store' })
  ...
  .catch(function () {
    // 加载失败不断言成功：保持关门（内存改动不同步文件），菜单里明示
    try { setSaveStatus('设置加载失败：本次修改仅生效到刷新，重进页面重试') } catch (e) {}
    refresh(false)
  })
      ...
      handler: async (req, res) => {
        if (req.method === 'PUT' || req.method === 'POST') {
          try {
            ...
            // 合并语义：缺字段沿用旧值，不再整包按默认重建。
            // 之前缺 scale 直接 400、其余缺字段按默认写，会把旧配置洗掉。
            const stored = readSizeConfig()
            const old = stored || {}
            const merged = {
              scale: typeof parsed.scale === 'number' ? parsed.scale : old.scale,
              sound: typeof parsed.sound === 'boolean' ? parsed.sound : old.sound,
              vol: typeof parsed.vol === 'number' ? parsed.vol : old.vol,
              soundSet: typeof parsed.soundSet === 'string' ? parsed.soundSet : old.soundSet,
              usageMode: typeof parsed.usageMode === 'string' ? parsed.usageMode : old.usageMode,
              peakMode: typeof parsed.peakMode === 'string' ? parsed.peakMode : old.peakMode,
              bubbleOn: typeof parsed.bubbleOn === 'boolean' ? parsed.bubbleOn : old.bubbleOn,
              turnCostOn: typeof parsed.turnCostOn === 'boolean' ? parsed.turnCostOn : old.turnCostOn,
              turnCostCloseMs: typeof parsed.turnCostCloseMs === 'number' ? parsed.turnCostCloseMs : old.turnCostCloseMs,
              scrollGapOn: typeof parsed.scrollGapOn === 'boolean' ? parsed.scrollGapOn : old.scrollGapOn,
              scrollGapPx: typeof parsed.scrollGapPx === 'number' ? parsed.scrollGapPx : old.scrollGapPx,
            }
            if (typeof merged.scale !== 'number') {
              res.writeHead(400, JSON_HEADERS)
              res.end(JSON.stringify({ ok: false, error: 'missing scale' }))
              return
            }
            // 用量模式变化时让余额缓存失效，下次请求立即按新模式计算
            if (!stored || normalizeUsageMode(old.usageMode) !== normalizeUsageMode(merged.usageMode)) {
              balanceCache = null
            }
            const result = writeSizeConfig(merged.scale, merged.sound !== false, merged.vol, merged.soundSet, merged.usageMode, merged.peakMode, merged.bubbleOn, merged.turnCostOn, merged.turnCostCloseMs, merged.scrollGapOn, merged.scrollGapPx)
